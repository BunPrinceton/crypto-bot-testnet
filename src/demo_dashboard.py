#!/usr/bin/env python3
"""
Demo Dashboard with Simulated Arbitrage Opportunities
...
latest_data = {
    ...
    'iteration': 0,
    'spread_data': [],
    'demo_mode': False  # Toggle for demo mode
}
...
def add_demo_variation(prices):
    """Add realistic price variations to create arbitrage opportunities"""
    if not prices or len(prices) < 2:
        return prices

    # Get base price (average)
    base_prices = [p['last'] for p in prices.values() if p]
    if not base_prices:
        return prices

    avg_price = sum(base_prices) / len(base_prices)

    # Add variations (0.3% to 1.5%)
    demo_prices = {}
    exchange_names = list(prices.keys())

    for i, exchange_name in enumerate(exchange_names):
        if prices[exchange_name]:
            # Create variation: some exchanges higher, some lower
            variation = random.uniform(-0.015, 0.015)  # -1.5% to +1.5%

            # Make sure at least 2-3 have enough spread
            if i == 0:
                variation = random.uniform(0.005, 0.015)  # Higher
            elif i == len(exchange_names) - 1:
                variation = random.uniform(-0.015, -0.005)  # Lower
...
def fetch_prices(symbol=SYMBOL, demo_mode=False):
    """Fetch prices, with optional demo variations"""
    prices = {}
    ...
    # Apply demo variations if in demo mode
    if demo_mode:
        prices = add_demo_variation(prices)
...
def update_data_loop():
    ...
    while True:
        try:
            ...
            prices = fetch_prices(demo_mode=demo_mode)
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("🎭 DEMO MODE CRYPTO ARBITRAGE DASHBOARD")
    print("="*60)
    print(f"Monitoring: {SYMBOL}")
    print(f"Exchanges: {', '.join(exchanges.keys())}")
    print(f"Fee: {FEE_PERCENT}%")
    print(f"Update Interval: {UPDATE_INTERVAL} seconds")
    print("\n💡 Click 'DEMO MODE' button to toggle simulated opportunities")
    print("="*60)
